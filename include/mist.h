 ...
 */
bool mist_apps_get_dlc_data_by_index(int32_t dlc, struct MistDlcData *dlc_data);

/**
 * Checks if an app with the appid is installed
 * Returns false on error
 */
bool mist_apps_is_app_installed(AppId app_id, bool *installed);

/**
 * Checks if the app is running in a cybercafe
 * Returns false on error
 */
bool mist_apps_is_cybercafe(bool *is_cybercafe);

/**
 * Checks if a dlc with the appid is installed
 * Returns false on error
 */
bool mist_apps_is_dlc_installed(AppId app_id, bool *installed);

/**
 * Checks if low violence mode is set
 * Returns false on error
 */
bool mist_apps_is_low_violence(bool *is_low_violence);

/**
 * Checks if the active user is subscribed to the current app
 * Returns false on error
 */
bool mist_apps_is_subscribed(bool *is_subscribed);

/**
 * Checks if the active user is subscribed to the app id
 * Returns false on error
 */
bool mist_apps_is_subscribed_app(AppId app_id, bool *is_subscribed);

/**
 * Checks if the active user is subscribed from family sharing
 * Returns false on error
 */
bool mist_apps_is_subscribed_from_family_sharing(bool *is_subscribed_from_family_sharing);

/**
 * Checks if the active user is subscribed from free weekend
 * Returns false on error
 */
bool mist_apps_is_subscribed_from_free_weekend(bool *is_subscribed_from_free_weekend);

/**
 * Checks if the user has a VAC ban
 * Returns false on error
 */
bool mist_apps_is_vac_banned(bool *is_vac_banned);

/**
 * Get the current build id of the application
 * Returns false on error
 */
bool mist_apps_get_app_build_id(BuildId *build_id);
 ...
 */
bool mist_apps_get_app_install_dir(AppId app_id, const char **app_install_dir);

/**
 * Get the steam id of the owner of the application
 * Returns false on error
 */
bool mist_apps_get_app_owner(SteamId *steam_id);

/**
 * Get a comma seperated list of the avaliable game languages
 * Returns false on error
 */
bool mist_apps_get_available_game_languages(const char **avaliable_languages);
 ...
 * Returns false on error
 */
bool mist_apps_get_current_beta_name(const char **current_beta_name);

/**
 * Get the current game language
 * Returns false on error
 */
bool mist_apps_get_current_game_language(const char **current_game_language);

/**
 * Get the dlc count used for getting the dlc info by index
 * Returns false on error
 */
bool mist_apps_get_dlc_count(int32_t *dlc_count);

/**
 * Get the download progress of a dlc
 * Returns false on error
 */
bool mist_apps_get_dlc_download_progress(AppId app_id,
                                         bool *downloading,
 ...
 * Returns false on error
 */
bool mist_apps_get_earliest_purchase_unix_time(AppId app_id, uint32_t *purchase_time);
 ...
 * Returns false on error
 */
bool mist_apps_get_installed_depots(AppId app_id,
                                    DepotId *depots,
                                    uint32_t depots_size,
                                    uint32_t *installed_depots);

bool mist_apps_get_launch_command_line(const char **launch_command_line);
 ...
 * Returns false on error
 */
bool mist_apps_get_launch_query_param(const char *key, const char **value);

/**
 * Request the dlc for the app id to be installed
 * Returns false on error
 */
bool mist_apps_install_dlc(AppId app_id);
 ...
 * Returns false on error
 */
bool mist_apps_mark_content_corrupt(bool missing_files_only);

/**
 * Request the dlc for the app id to be uninstalled
 * Returns false on error
 */
bool mist_apps_uninstall_dlc(AppId app_id);

/**
 * Clears the rich presence key/value store
 * Returns false on error
 */
bool mist_friends_clear_rich_presence(void);
 ...
 * Returns false on error
 */
bool mist_friends_set_rich_presence(const char *key, const char *value);

/**
 * Returns the appid of the running application
 * Returns
 */
bool mist_utils_get_appid(uint32_t *app_id);
